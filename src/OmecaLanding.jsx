  ...
  return (
    <Box sx={{ width: "100%", maxWidth: 600, mx: "auto" }}>
      ...
      <Grid container spacing={2}>
        <Grid item xs={12} sm={6}><HeroMetricCard label="L1: Operational Integrity" sublabel="Verified Inputs" metric={data.control} color={colors.accent} /></Grid>
        <Grid item xs={12} sm={6}><HeroMetricCard label="L2: Continuous Close" sublabel="Real Time Reconciliation" metric={data.recon} color={colors.successGreen} /></Grid>
        <Grid item xs={12}><HeroMetricCard label="L3: Autonomous Governance" sublabel="Enforced Control" metric={data.evidence} color={colors.lucraGold} icon={VerifiedUser} /></Grid>
            ...
            <Grid container spacing={{ xs: 8, md: 6 }} alignItems="center">
              <Grid size={{ xs: 12, md: 6 }}>
                <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }}>
                  ...
                  <Typography variant="h1" sx={{ fontWeight: 800, fontSize: { xs: "3rem", md: "4.8rem" }, letterSpacing: "-0.03em", lineHeight: 1.1, mb: 3, background: isDark ? `linear-gradient(180deg, #fff 0%, rgba(255,255,255,0.7) 100%)` : `linear-gradient(180deg, #000 0%, rgba(0,0,0,0.7) 100%)`, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>The New <br /><Box component="span" sx={{ background: `linear-gradient(90deg, ${colors.accent}, ${colors.lucraGold})`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Financial Core.</Box></Typography>
                  <Typography variant="h6" sx={{ color: currentColors.textDim, mb: 5, maxWidth: 580, lineHeight: 1.6, fontWeight: 400 }}>Transforming ERPs from passive record keeping to continuous autonomous control.
                    Unify operational truth, real time close, and verifiable intelligence.</Typography>
                  ...
                  {user ? (
                    ...
                    <Box component="form" onSubmit={handleStartFlow} sx={{ display: 'flex', alignItems: 'center', maxWidth: 440, bgcolor: isDark ? 'rgba(255,255,255,0.05)' : '#fff', border: '1px solid', borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)', borderRadius: 50, p: 0.5, pl: 2.5, boxShadow: isDark ? 'none' : '0 4px 20px rgba(0,0,0,0.05)', transition: 'all 0.2s', "&:focus-within": { borderColor: colors.accent, boxShadow: `0 0 0 4px ${colors.accent}20` } }}>
                      ...
                      />
                      <Button type="submit" sx={{ borderRadius: 50, px: 3, py: 1.2, bgcolor: colors.accent, color: '#000', fontWeight: 700, textTransform: 'none', minWidth: 'fit-content', whiteSpace: 'nowrap', "&:hover": { bgcolor: '#fff' } }}>Start Your Pilot</Button>
                    </Box>
                  )}
                  ...
                  <Box sx={{ mt: 8, opacity: 0.7, maxWidth: 500, overflow: 'hidden', position: 'relative', maskImage: 'linear-gradient(to right, transparent, black 20%, black 80%, transparent)' }}>
                    <Typography variant="caption" sx={{ color: currentColors.textDim, fontWeight: 700, mb: 1.5, display: 'block', fontSize: '0.65rem', letterSpacing: 1, textTransform: 'uppercase' }}>Connect Your Systems to Omeca</Typography>
                    <motion.div animate={{ x: ["0%", "-50%"] }} transition={{ duration: 20, ease: "linear", repeat: Infinity }} style={{ display: 'flex', gap: '2rem', whiteSpace: 'nowrap' }}>{[
                      'OpenAI',
                      'Anthropic',
                      'LangChain',
                      'Stripe',
