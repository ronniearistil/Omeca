...
const AtomicAuditLogModal = ({ isOpen, handleClose, entry }) => {
    ...
    // FIX: Using chain_id for transaction check
    const isTransaction = entry && entry.chain_id !== undefined;
    ...
    const renderContent = () => {
        const finalDestination = entry.status === 'Audited' ? 'GL (General Ledger) Chained' : 'Quarantine Queue';
...
// --- THE MAIN DASHBOARD COMPONENT -----------------------
// --------------------------------------------------------
const OmecaDashboard = () => { // COMPONENT NAME REMAINS OmecaDashboard
    const { mode, toggleColorMode } = useContext(ColorModeContext);

    // --- State Management ---
    const [ledgerData, setLedgerData] = useState(agentPool.slice(0, 5).map(entry => ({ ...entry, id: Date.now() + Math.random() })));
    const [unauditedSpend, setUnauditedSpend] = useState(0.0);
    const [totalSpend, setTotalSpend] = useState(4.20); // Not used in final metric calculation, retained for simulation logic consistency
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            setLedgerData(prevData => {
                ...
                // 1. Dynamic Total Spend Update logic retained for simulation consistency
                // setTotalSpend(prev => +(prev + newEntry.cost).toFixed(2));

                // 2. Simulate Audit Leakage update based on stream
                if (newEntry.status !== 'Audited') {
                    setUnauditedSpend(prev => Math.min(prev + 0.05, 0.15));
                } else if (prevData.length >= 7) {
                    setUnauditedSpend(prev => Math.max(prev - 0.01, 0.0));
                }
        ...
        >
            ...
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 5 }}>
                ...
                <Box sx={{ textAlign: 'left' }}>
                    <Typography variant="h4" fontWeight={900}>
                        <GradientText variant="h4" sx={{ fontSize: 'inherit' }}>
                            Omeca {/* UNIFIED BRAND: Omeca */}
                        </GradientText>
                    </Typography>
                    <Typography variant="h5" sx={{ color: colors[mode].textPrimary, fontWeight: 700 }}>
                        The Omeca Trust Stack: Financial Control Core {/* UNIFIED MESSAGE: Single product identity */}
                    </Typography>
                    ...
                    >
                        Prototype demonstration of Omeca’s immutable, explainable audit layer {/* UNIFIED BRAND: Omeca */}
                    </Typography>
                </Box>
                        ...
                        <motion.div variants={fadeInUp} style={{ height: '100%' }}>
                            <Paper sx={{ p: 4, height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                                <Box>
                                    ...
                                    <Typography variant="body1" color="textSecondary" sx={{ mb: 3 }}>
                                        {/* UNIFIED MESSAGE: Omeca links... not Melucra links... */}
                                    ...
                                    <motion.div variants={staggerContainer}>
                                        ...
                                        <AllocationBar
                                            ...
                                            color="lucraGold"
                                            value={quarantinedSpend.toFixed(2)}
                                            handleSubitemClick={handleDrillDown} // Assuming this function exists or using handleDrillDown
                                        />
                                        <AllocationBar
                                            name="Explainability Coverage"
                                            percentage={explainabilityCoverage}
...
// Final component for export, wrapped in the local ThemeProvider
const OmecaDashboardPage = () => ( // COMPONENT NAME REMAINS OmecaDashboardPage
    <ThemeWrapper>
        <OmecaDashboard />
    </ThemeWrapper>
);
