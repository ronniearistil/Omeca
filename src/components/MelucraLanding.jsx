...
import MelucraDashboardPage from "./pages/MelucraDashboardPage.jsx";

import MelucraDashboardPreview from './pages/MelucraDashboardPreview.jsx';
import MelucraWaitlistDialog from './MelucraWaitlistDialog';
...
const PricingPage = ({ setPage }) => {
    ...
    const tiers = [
        ...
        { name: "Growth", price: "Integration Access Tier", desc: "For scaling teams that need robust tracking and basic alerts.", features: ["Up to 50M events/month", "Predictive Margin Alerts", "Priority support"] },
            ...
            <AppBar position="static" elevation={0} sx={{ bgcolor: 'transparent', pt: 1 }}>
                <Toolbar sx={{ justifyContent: 'space-between' }}>
                    ...
                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                        <ThemeToggleButton />
                        <Button variant="outlined" size="medium" sx={{ borderColor: colors.accent, color: colors.accent, fontWeight: 700, textTransform: 'none', '&:hover': { bgcolor: `${colors.accent}14`, borderColor: colors.accent } }}
                            // FIX: Use stable handler to focus input without forced scrolling
...
// // src/components/MelucraLanding.jsx
// import React, { useState, useContext, useRef } from 'react';
// import { Box, AppBar, Toolbar, Typography, Button, TextField, Container } from '@mui/material';
// import { motion } from 'framer-motion';
// import ArrowForwardIosRounded from '@mui/icons-material/ArrowForwardIosRounded';
// 
// // Core Imports
// import { ColorModeContext } from '../layouts/theme/ThemeContext'; // Adjusted path
// import { colors } from '../layouts/theme/theme'; // Adjusted path
// import ThemeToggleButton from '../layouts/ThemeToggleButton'; // Adjusted path
// import MelucraLogoComponent from './ui/MelucraLogo';
// import AppFooter from './ui/AppFooter';
// import MelucraAppRouter from './MelucraAppRouter'; // Import the router
// 
// // Note: Removed definitions for colors, getDesignTokens, Context, Logo, Footer, Router, Pages, Sections
// 
// const MelucraLanding = () => {
//     const { mode } = useContext(ColorModeContext);
//     const currentColors = colors[mode];
...
// 
//     // State and Ref for the hero email form
//     const [email, setEmail] = useState('');
//     const [submitted, setSubmitted] = useState(false);
//     const emailInputRef = useRef(null);
// 
...
//         } else {
//             // Simple inline feedback or use a more robust validation library
//             if (emailInputRef.current) {
//                 emailInputRef.current.focus(); // Focus the input on error
//             }
//             alert("Please enter a valid email address."); // Placeholder for better error handling
//             setSubmitted(false);
//         }
//     };
// 
//     // Handler for the "Request Invite" button in AppBar to focus email input
//     const handleRequestInviteClick = () => {
//         if (emailInputRef.current) {
//             emailInputRef.current.scrollIntoView({ behavior: 'smooth', block: 'center' });
//             setTimeout(() => {
...
//                     emailInputRef.current.focus();
//                 }
//             }, 300); // Delay focus slightly after scroll
//         }
//         // Ensure we are on the home page to see the form
//         if (page !== 'home') {
//             setPage('home');
//         }
//     };
...
// 
//     return (
//         // Root Box using theme colors
//         <Box sx={{
//             bgcolor: currentColors.bgTop,
//             color: currentColors.textPrimary,
//             minHeight: '100vh',
//             display: 'flex', // Added for potential sticky footer pattern later
//             flexDirection: 'column' // Added for potential sticky footer pattern later
//         }}>
// 
//             {/* --- Top Navigation Bar --- */}
//             <AppBar position="static" elevation={0} sx={{ bgcolor: 'transparent', pt: 1 }}>
//                 <Container maxWidth="lg"> {/* Wrap Toolbar in Container for consistent width */}
//                     <Toolbar sx={{ justifyContent: 'space-between', px: { xs: 0, sm: 2 } }}> {/* Adjust padding */}
//                         {/* Logo and Brand Name */}
//                         <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, cursor: 'pointer' }} onClick={() => setPage('home')}>
//                             <MelucraLogoComponent size={36} />
//                             <Typography variant="h6" fontWeight={800} letterSpacing={0.5} color={currentColors.textPrimary} sx={{ display: { xs: 'none', sm: 'block' } }}>
//                                 MELUCRA
//                             </Typography>
//                         </Box>
//                         {/* Right side controls */}
...
//                                 size="medium"
//                                 sx={{
//                                     borderColor: colors.accent, color: colors.accent, fontWeight: 700, textTransform: 'none',
//                                     '&:hover': { bgcolor: `${colors.accent}14`, borderColor: colors.accent }
//                                 }}
//                                 onClick={handleRequestInviteClick} // Focus email input
//                             >
//                                 Request Invite
//                             </Button>
//                         </Box>
...
// 
//             {/* --- Main Content Area --- */}
//             {/* Conditional rendering for Hero section ONLY on 'home' page */}
//             {page === 'home' && (
//                 <Box sx={{
//                     pt: { xs: 6, md: 8 }, // Adjusted padding
//                     pb: { xs: 4, md: 6 },
//                     px: { xs: 2, sm: 0 }, // No horizontal padding on container for sections inside
//                     overflowX: 'hidden',
//                     textAlign: 'center',
//                     background: `radial-gradient(900px 400px at 50% -10%, ${currentColors.bgGradA} 0%, ${currentColors.bgTop} 60%), linear-gradient(180deg, ${currentColors.bgGradB} 0%, ${currentColors.bgTop} 40%)`
//                 }}>
//                     <Container maxWidth="lg"> {/* Container for hero content alignment */}
//                         <motion.div initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
//                             {/* Brand Header */}
//                             <Typography component="h1" sx={{ /* ... styles from original */ }}>
//                                 Melucra
//                             </Typography>
//                             {/* Main Hero Headline */}
//                             <Typography variant="h3" fontWeight={900} sx={{ /* ... styles from original */ }}>
//                                 The Explainability Core powering the{" "}
//                                 <span style={{
//                                      background: `linear-gradient(90deg, ${colors.accent}, ${colors.lucraGold})`,
//                                      WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", display: "inline-block"
//                                  }}>$1.3&nbsp;Trillion</span> Machine Economy
//                             </Typography>
//                              {/* Sub-headline */}
//                              <Typography variant="h5" align="center" sx={{ /* ... styles from original */ }}>
//                                 <strong>Melucra</strong> captures every action...
//                              </Typography>
//                         </motion.div>
// 
//                         {/* Inline Email Form */}
//                          <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.35, duration: 0.45 }}>
//                             <Box component="form" onSubmit={handleSubmit} sx={{ /* ... styles for flex layout */ }} id="invite-form-target">
//                                 {!submitted ? (
//                                     <>
//                                         <TextField
//                                             variant="outlined" placeholder="Your work email" type="email" value={email} onChange={(e) => setEmail(e.target.value)}
//                                             inputRef={emailInputRef}
//                                             InputProps={{ sx: { /* ... styles */ } }} sx={{ '& .MuiOutlinedInput-notchedOutline': { border: 'none' } }}
//                                         />
//                                         <Button type="submit" variant="contained" size="large" endIcon={<ArrowForwardIosRounded />} sx={{ /* ... styles */ }}>
//                                             Get Started
//                                         </Button>
//                                     </>
//                                 ) : (
...
//              )}
// 
//             {/* Router View - Renders sections on home, or specific pages */}
//             {/* Container is removed here, individual pages/sections should manage their own */}
//             <Box sx={{ flex: 1 }}> {/* Added flex: 1 to push footer down */}
//                 <MelucraAppRouter currentPage={page} setPage={setPage} />
//             </Box>
// 
// 
//             {/* --- Footer --- */}
//             <AppFooter setPage={setPage} /> {/* Pass setPage down */}
// 
//         </Box>
//     );
// };
// 
// export default MelucraLanding;